...
#       not POSIX-compliant, you can try to use /usr/bin/env sh or dash.
#   (2) This script is not intended to be run in a Windows environment.
#
##############################################################################
...
# Need this for albumfs-fuse on 64-bit Ubuntu
while [ -h "$app_path" ]; do
    ls=`ls -ld "$app_path"`
    link=`expr "$ls" : '.*-> \(.*\)$'`
...
if [ -n "$JAVA_HOME" ] ; then
    if [ -x "$JAVA_HOME/bin/java" ] ; then
        # Now, $JAVA_HOME is the directory of the Java runtime
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # Now use the pattern with cygpath to convert any paths to Windows format in the arguments list
    for arg in "$@" ; do
    ...
    # Collect the arguments into a variable of the form: "arg1" "arg2" "arg3"
    for arg in "$@" ; do
